...
class SQLParserSpec extends FlatSpec with Matchers {
  ...
  it should "parse " + w0 in {
    val expected = InvocationSelect(List[Invocation](), Scalar("t"), None, None)
  ...
  it should "parse " + w1 in {
    val expected = InvocationSelect(List(InvocationColumn(InvocationP(Left("a")), None)), "t", None, None)
  ...
  it should "parse " + w2 in {
    val expected = InvocationSelect(List(InvocationColumn(InvocationP(Left("a")), Some("b"))), "t", None, None)
  ...
  it should "parse " + w3 in {
    val expected = InvocationSelect(List(InvocationColumn(InvocationP(Left(Scalar("a"))), Some(Scalar("b")))), Scalar("t"), Some(InvocationBooleanExpression(Right(InvocationComparison(Right(InvocationLookup("d")), "=", Right(InvocationLookup("e")))), List())), None)
    parser.parseAll(parser.select, w3) should matchPattern { case parser.Success(`expected`, _) => }
  }

  it should "parse " + w4 in {
    val expected = InvocationSelect(List[Invocation](), "t", None, Some(100))
    parser.parseAll(parser.select, w4) should matchPattern { case parser.Success(`expected`, _) => }
  }

  it should "parse " + w5 in {
    val expected = InvocationSelect(List(InvocationColumn(InvocationP(Left("a")), None), InvocationColumn(InvocationCaseClause(List(InvocationWhenThen(InvocationBooleanExpression(Left(true), List()), Left(42))), None), None)), "t", None, None)
